...
def loadimg(img):  # loading opencv images
    img_size=640
    cap=None
    path=None
    img0 = img
...
def publish_pred_array(tensor):
    if tensor is not None:
        tensor = torch.flatten(tensor)  # pulisher only accept 1D array
        pred_array = Float64MultiArray()
        pred_array.data = tensor.cpu().numpy().astype(np.float64)
        pred_pub.publish(pred_array)
...
if __name__ == '__main__':
    ...
    image_topic = "/color/image_rect"
   
    rospy.Subscriber(image_topic, Image, image_callback, queue_size=1, buff_size=52428800)

    image_pub = rospy.Publisher('/yolo_img_result', Image, queue_size=10)
    pred_pub = rospy.Publisher('/yolo_pred_result', Float64MultiArray, queue_size=10)
